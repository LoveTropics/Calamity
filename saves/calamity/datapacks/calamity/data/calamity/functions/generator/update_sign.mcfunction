...
execute at @s[tag=Charging] at @s run data merge block ^ ^ ^3 {Text3: '{"translate":"calamity.generator.sign.progress","with":[{"score":{"name":"@e[current_world=true,limit=1,distance=..4,type=minecraft:area_effect_cloud]","objective":"displayPercent"}}]}'}
# Update sign with resetting percentage
execute at @s[tag=Resetting] at @s run data merge block ^ ^ ^3 {Text3: '{"translate":"calamity.generator.sign.resetting","with":[{"score":{"name":"@e[current_world=true,limit=1,distance=..4,type=minecraft:area_effect_cloud]","objective":"displayPercent"}}]}'}
