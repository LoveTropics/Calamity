...
function calamity:player/trigger/ready_team
execute if score ReadyBlue gameVariable matches 1 unless entity @a[current_world=true,team=blue] run function calamity:game_state/ready_team/blue_not_ready
execute if score ReadyRed gameVariable matches 1 unless entity @a[current_world=true,team=red] run function calamity:game_state/ready_team/red_not_ready
...
scoreboard players set @a[current_world=true,scores={startMatch=..-1}] startMatch 0
scoreboard players enable @a[current_world=true,scores={startMatch=0}] startMatch
# If a game is not starting, check to see if players want to start a game
execute as @a[current_world=true,scores={startMatch=1..}] at @s run function calamity:player/trigger/start_match

# If a game start is happening, check to see if players want to cancel it
execute as @a[current_world=true,scores={cancelStart=1..}] at @s if score StartingMatch gameVariable matches 1 run function calamity:player/trigger/cancel_start
...
function calamity:player/trigger/join_team
function calamity:player/trigger/spectate
function calamity:player/trigger/arena
